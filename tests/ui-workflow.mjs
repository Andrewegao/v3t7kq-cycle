...
test('staging workflow leaves release-mode activation to the exact-profile controller',()=>{
  ...
  const stagedController="ref: ${{ needs.profile.outputs.model_selection_sha256 == 'none' && 'dbc97a26bc239398ffa9ec157a094148961b6451' || '954a6ac1bd0ea0f7617f7f521c725a22ff64f1c7' }}";
  assert.equal(staging.split(stagedController).length-1,2);
  assert.match(candidate,/STAGING_CONTROL_SHA = '954a6ac1bd0ea0f7617f7f521c725a22ff64f1c7'/);
  assert.match(prod,/ref: dbc97a26bc239398ffa9ec157a094148961b6451/);
  assert.doesNotMatch(prod,/954a6ac1bd0ea0f7617f7f521c725a22ff64f1c7/);
});
