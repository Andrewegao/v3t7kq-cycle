...
test('scheduler CI runs inventory checks and selects all workflow changes and metadata', async () => {
  ...
  assert.equal(data.jobs.scheduler.steps[index].run.trim(), 'node --test tests/workflow-inventory.mjs tests/workflow-timing.mjs tests/platform-staging-transaction.mjs tests/production-account-audit.mjs\nnode tools/workflow-inventory.mjs --check');
