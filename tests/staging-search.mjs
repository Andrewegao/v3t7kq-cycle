...
import { candidate, validateIndex, prepareSearch, activateSearch, renewSearch, revokeSearch, inspectSearch,
  verifyV4Staging, hash, POINTER_KEY, searchGate, allowedSearchKey } from '../tools/staging-search.mjs';
import { ATMOS_SHA, STAGING_ORIGIN } from '../tools/staging-search-source.mjs';
...
test('manual staging workflow keeps writer secrets out of checkout, build and tests', () => {
  ...
  assert.ok(writer > 0);
  assert.ok(compatibility > 0 && compatibility < writer);
  ...
  assert.ok(!/^0{40}$/.test(ATMOS_SHA) && workflow.includes(`ref: ${ATMOS_SHA}`));
...
test('gate refuses local/unreviewed execution, foreign credentials and weather writes', () => {
  ...
  const activate = { ...env, CANDIDATE_SHA256: 'a'.repeat(64), STAGING_SEARCH_APPROVED_CANDIDATE_SHA256: 'a'.repeat(64),
    EXPECTED_POINTER_SHA256: 'absent', STAGING_SEARCH_V4_APPROVED_SOURCE_SHA: ATMOS_SHA,
    STAGING_SEARCH_V4_APPROVED_RELEASE_ID: `git-${ATMOS_SHA.slice(0, 12)}-run-123` };
  searchGate(activate, 'activate');
  for (const key of ['STAGING_SEARCH_APPROVED_CANDIDATE_SHA256', 'STAGING_SEARCH_V4_APPROVED_SOURCE_SHA',
...
test('V2 activation accepts only the approved canonical V4 staging release', async () => {
  const releaseId = `git-${ATMOS_SHA.slice(0, 12)}-run-123`;
  const html = Buffer.from('<!doctype html><title>WeatherX</title><div id="root"></div>');
  const env = { STAGING_SEARCH_V4_APPROVED_SOURCE_SHA: ATMOS_SHA,
    STAGING_SEARCH_V4_APPROVED_RELEASE_ID: releaseId };
  const receipt = { releaseId, gitSha: ATMOS_SHA, shellSha256: 'b'.repeat(64), indexSha256: hash(html),
    buildProfile: { product: 'lab', platformAccount: '1', platformDataAuth: 'public' } };
  const fetcher = async (url, init) => {
    ...
    if (url === `${STAGING_ORIGIN}/health/release.json?search_v4_compatibility=1`) return Response.json(receipt);
    assert.equal(url, `${STAGING_ORIGIN}/?search_v4_compatibility=1`);
    return new Response(html, { headers: { 'content-type': 'text/html; charset=utf-8' } });
  };
  assert.deepEqual(await verifyV4Staging(env, fetcher), { releaseId, sourceSha: ATMOS_SHA });
  for (const change of [
    { env: { ...env, STAGING_SEARCH_V4_APPROVED_SOURCE_SHA: 'a'.repeat(40) } },
    { env: { ...env, STAGING_SEARCH_V4_APPROVED_RELEASE_ID: 'other' } },
    { response: Response.json({ ...receipt, gitSha: 'a'.repeat(40) }) },
    { response: Response.json({ ...receipt, releaseId: 'other' }) },
    { response: Response.json({ ...receipt, buildProfile: { ...receipt.buildProfile, product: 'road' } }) },
    ...
    await assert.rejects(verifyV4Staging(change.env ?? env,
      change.response ? async (url) => url.includes('/health/release.json') ? change.response :
        new Response(html, { headers: { 'content-type': 'text/html' } }) : fetcher));
  }
  await assert.rejects(verifyV4Staging(env, async (url) => url.includes('/health/release.json')
    ? Response.json({ ...receipt, indexSha256: 'c'.repeat(64) })
