...
test('Wind100 pin is optional, exact, and restricted to the account-standard staging profile',()=>{
  ...
  for(const env of [
    ...
    wind100Env({STAGING_WIND100_UI_DYNAMIC:'false'}),
    wind100Env({STAGING_WIND100_UI_DYNAMIC:'1'}),
  ...
  assert.throws(()=>resolveWind100BuildPin(ACCOUNT_CORE_PROFILE,{STAGING_WIND100_UI_DYNAMIC:'true'}),/disabled/);
  for(const value of [{...WIND100,dynamic:false},{...WIND100,dynamic:'true'},{...WIND100,dynamic:true,extra:true}])
    assert.throws(()=>validateWind100BuildProfile(value));
});
test('Wind100 build flags and receipt are exact while every disabled profile clears ambient values',()=>{
  const inherited={VITE_STAGING_WIND100:'1',VITE_STAGING_WIND100_CATALOG_ID:'malicious',VITE_STAGING_WIND100_RUN_ID:'malicious',
    VITE_STAGING_WIND100_SELECTION_SHA256:'malicious',VITE_STAGING_WIND100_DYNAMIC:'1'};
  ...
  assert.equal(enabled.VITE_STAGING_WIND100_DYNAMIC,'');
  ...
  const dynamicEnv=wind100Env({STAGING_WIND100_UI_DYNAMIC:'true'}),dynamicReceipt=structuredClone(receipt);
  dynamicReceipt.buildProfile.wind100={...WIND100,dynamic:true};
  const dynamicBuild=publicBuildEnvironment(ACCOUNT_CORE_PROFILE,null,dynamicEnv);
  assert.equal(dynamicBuild.VITE_STAGING_WIND100_DYNAMIC,'1');
  assert.equal(dynamicBuild.STAGING_WIND100_UI_DYNAMIC,undefined);
  assert.deepEqual(validateWind100BuildReceipt(ACCOUNT_CORE_PROFILE,dynamicReceipt,dynamicEnv),{...WIND100,dynamic:true});
  assert.throws(()=>validateWind100BuildReceipt(ACCOUNT_CORE_PROFILE,dynamicReceipt,wind100Env()));
  assert.throws(()=>validateWind100BuildReceipt(ACCOUNT_CORE_PROFILE,receipt,dynamicEnv));
  ...
  for(const profile of [BASELINE_PROFILE,CORE_RELEASE_PROFILE]){
    ...
    for(const key of ['VITE_STAGING_WIND100','VITE_STAGING_WIND100_CATALOG_ID','VITE_STAGING_WIND100_RUN_ID','VITE_STAGING_WIND100_SELECTION_SHA256','VITE_STAGING_WIND100_DYNAMIC'])assert.equal(off[key],'');
