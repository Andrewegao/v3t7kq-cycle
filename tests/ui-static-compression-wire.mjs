...
async function request(url,{headers,method}){
  ...
  if(headers['If-None-Match'])return response(304,Buffer.alloc(0),{etag:entry.etag});
  if(headers['Accept-Encoding']==='identity')return response(200,raw);
  return response(200,method==='HEAD'?Buffer.alloc(0):br,{'content-encoding':'br','content-type':entry.mime,etag:entry.etag,vary:'Accept-Encoding','x-content-type-options':'nosniff','cache-control':'public, max-age=31536000, immutable, no-transform','cloudflare-cdn-cache-control':'no-store'});
}
