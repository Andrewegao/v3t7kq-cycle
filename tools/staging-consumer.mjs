...
export function desiredSettings(before) {
  ...
  assert.equal(seen.size,2,'staging mode bindings missing');return desired;
}
...
export function assertAllowedTransition(config,before,after) {
  const wanted=desiredSettings(before);assertSettings(config,wanted);
  assert.deepEqual(after,wanted,'staging change exceeds the two approved public-mode bindings');
}
...
export async function main(command,atmos,receiptPath,env=process.env){
  ...
  if(command==='preflight'){
    ...
    await persist(receipt);console.log(JSON.stringify({worker:WORKER,version:before.version,settingsSha256:receipt.settingsSha256,changes:['AUTH_MODE=public','BILLING_MODE=disabled'],deployed:false}));return receipt;
  }
  consumerGate(env);
