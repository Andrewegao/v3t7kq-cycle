...
assert.match(bakeWorkflow, /cron: '30 2,8,14,20 \* \* \*'/,
  'staging Wind100 must retain its independent GitHub-native fallback');
...
assert.match(deployWorkflow, /working-directory: scheduler[\s\S]*?run: npm run verify:live/,
  'guarded deploy must verify the live trigger set and production target');
