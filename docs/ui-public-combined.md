...
The source pin in `tools/ui-public-combined.mjs`, the source guard pin in `tools/ui-combined-source-guard.mjs`, and all three literal controller refs in the protected UI workflows identify reviewed Atmos master commit `9da64b54fadf6efb21302c1504575c75bf5b1d54`. This commit scopes the account qualification unavailable-state selector so the onboarding session-check panel cannot satisfy the terminal timeout assertion; it does not change application runtime code. The repair follows the automatic rollback in staging run `36225198580`; release fuse issue `321` remains open until immediately before the authorized guarded retry. Keep all five operational literals identical; a later source change requires a fresh review and candidate qualification. The protected `ui-staging` environment must set `UI_PRODUCTION_ACCOUNT_PROFILE_APPROVED=production-account-billing-v1`, `UI_PUBLIC_LOCALE_BETA_PROFILE_APPROVED=production-account-ru-kk-beta-v1`, and `UI_PUBLIC_COMBINED_PROFILE_APPROVED=production-account-ru-kk-wind100-onboarding-v2`; the existing UI build, Pages, source-read, and candidate credentials and reviewer gates still apply. No approval variable alone can bypass the source pin.
