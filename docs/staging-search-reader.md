...
动态 Wind100 切换时，若现场仍有完整的旧版固定三元组，控制器保留该三元组并只开启动态标记，让已打开的页面在原始有效期内继续读取。新候选资格探针仍严格检查本次批准的动态三元组；它不改用旧数据通过检查。此切换必须配合支持旧版精确请求且检查原始有效期的 Worker 源码。没有旧版绑定时直接使用已批准的新三元组；其他绑定及回退保护不变。
