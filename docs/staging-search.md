...
- producer SHA `dfa25e9f473f15d5e2f630fe78c268b73234bd3a` 固定在工具及 workflow，只用于生成 V2 候选及其来源证明；它不是 UI 发布 SHA。五类动态目录必须来自同一个明确的不可变 release；城市表、台风名及拼音表、机场中文审核表来自同一固定 producer checkout。构建器只用 Python 标准库，不安装依赖、不触发外部天气采集。
...
4. 先部署并用真实浏览器验证固定 SHA 的 Search V4 壳仍可读取当前 V1 对；当前 Search V3 明确拒绝 V2，禁止先切共享指针。把浏览器资格审查过的 canonical staging `releaseId` 与**精确 UI 发布 SHA**分别写入受保护环境 allowlist `STAGING_SEARCH_V4_APPROVED_RELEASE_ID`、`STAGING_SEARCH_V4_APPROVED_UI_SOURCE_SHA`。激活时另行 checkout 该精确 UI SHA，要求工作树干净、producer SHA 是其祖先，并核对 `Search.tsx` 及九个 Search 读取器/规范化依赖（包括从壳层抽出的 `searchSources.ts`）的固定 SHA-256 closure；不得把任意后续 main SHA 当作等价读取器。`activate` 随后在取得写凭据之前读取 `/health/release.json`，要求现场 `gitSha`、`releaseId`、shell/index 摘要精确相等。不存在同时兼容 V3/V4 的双格式或版本化 URL 通道。
...
激活默认 24 小时；协议最多 48 小时。读取器缓存最多 4 个小指针、30 秒，且不得超出有效期；撤销传播最多一个缓存期限。过期恢复旧路径，旧路径缺搜索资料时仍会显示资料不可用，不冒充永久修复。既有 V1 候选按原合同独立续租，不需要 V4 allowlist；V2 候选每次续租都重新核验当时受保护的精确 UI SHA 与现场 canonical release，撤销/回滚/换版后不会静默复活。canonical account staging receipt 只允许基础 `{product:lab, platformAccount:1, platformDataAuth:public}`，或由 UI 发布通道独立批准的精确 `wind100` catalog/run/selection 元组；本通道的十文件 closure **不证明 Wind100 读取器兼容性**，只容许其已由 UI 通道认证的 profile 与 Search V4 共存。该通道是限时预发布资格验证，**没有定时刷新**；长期目录发布与其他资料的新鲜度需要另行方案。
